...
import {
    ...
    DIRECTION_NAMES,
    DIRECTION_NAMES_3D,
    DIRECTION_NAMES_3D_INVERSE,
    DIRECTION_NAMES_INVERSE,
    TIME_NAMES,
} from '../constants/engine.constants';
import { EnumScreens, EnumTiers } from '../models/syllogimous.models';
import {
    TIER_SCORE_ADJUSTMENTS,
    TIER_SCORE_RANGES,
    TIER_SETTINGS,
} from '../constants/syllogimous.constants';
import {
    LS_DONT_SHOW,
    LS_HISTORY,
    LS_SCORE,
...
export class SyllogimousService {
    ...
    set timerMultiplier(value: number) {
        this._timerMultiplier = value;
        localStorage.setItem(LS_TIMER_MULTIPLIER, JSON.stringify(value));
    }

    constructor(private modalService: NgbModal) {
        this.loadScore();
        this.loadHistory();
        this.loadTimerMultiplier();
    ...
    loadTimerMultiplier() {
        const lsMultiplier = localStorage.getItem(LS_TIMER_MULTIPLIER);
        if (lsMultiplier) {
            this._timerMultiplier = JSON.parse(lsMultiplier);
        }
    }
    ...
    createQuestion() {
        ...
        if (this.settings.distinction[0]) {
            choices.push(() =>
                this.createDistinction(this.settings.distinction[1]),
            );
        }
        if (this.settings.comparisonNumerical[0]) {
            choices.push(() =>
                this.createComparison(
                    this.settings.comparisonNumerical[1],
        ...
        if (this.settings.comparisonChronological[0]) {
            choices.push(() =>
                this.createComparison(
                    this.settings.comparisonChronological[1],
        ...
        if (this.settings.syllogism[0]) {
            choices.push(() =>
                this.createSyllogism(this.settings.syllogism[1]),
            );
        }
        if (this.settings.direction[0]) {
            choices.push(() =>
                this.createDirection(this.settings.direction[1]),
            );
        }
        if (this.settings.direction3D[0]) {
            choices.push(() =>
                this.createDirection3D(this.settings.direction3D[1]),
            );
        }
        if (this.settings.direction4D[0]) {
            choices.push(() =>
                this.createDirection4D(this.settings.direction4D[1]),
            );
        }
    ...
    checkQuestion(value?: boolean) {
        ...
        if (this.question.userAnswer === this.question.isValid) {
            ...
            this.score = Math.max(
                0,
                this.score - TIER_SCORE_ADJUSTMENTS[this.tier].decrement,
            );
            if (this.score !== 0) {
                ds -= 1;
            }
        ...
        if (currTier !== nextTier) {
            const modalRef = this.modalService.open(ModalLevelChangeComponent, {
                centered: true,
            });
            if (ds > 0) {
                // level up
    ...
    createSyllogism(length: number) {
        ...
        do {
            ...
            isPremiseLikeConclusion(question.premises, question.conclusion)
        );
    ...
    createDirection(length: number) {
        ...
        while (!conclusionDirection) {
            ...
            for (let i = 0; i < words.length - 1; i++) {
                const dirIndex =
                    1 +
                    Math.floor(Math.random() * (DIRECTION_NAMES.length - 1));
                ...
                if (this.settings.enableNegation && coinFlip()) {
                    question.premises.push(
                        `<span class="subject">${words[i + 1]}</span> is at <span class="is-negated">${(DIRECTION_NAMES_INVERSE as any)[dirName]}</span> of <span class="subject">${words[i]}</span>`,
                    );
                } else {
                    question.premises.push(
            ...
            conclusionDirection = findDirection(
        ...
        question.isValid = coinFlip();
        const oppositeDirection = findDirection(
            wordCoordMap[words[length - 1]],
            wordCoordMap[words[0]],
        );
        const direction = question.isValid
            ? conclusionDirection
            : oppositeDirection;
    ...
    createDirection3D(length: number) {
        ...
        while (!conclusionDirection) {
            ...
            for (let i = 0; i < words.length - 1; i++) {
                const dirIndex =
                    1 +
                    Math.floor(Math.random() * (DIRECTION_NAMES_3D.length - 1));
                ...
                if (this.settings.enableNegation && coinFlip()) {
                    question.premises.push(
                        `<span class="subject">${words[i + 1]}</span> is <span class="is-negated">${(DIRECTION_NAMES_3D_INVERSE as any)[dirName]}</span> of <span class="subject">${words[i]}</span>`,
                    );
                } else {
                    question.premises.push(
    ...
    createDirection4D(length: number) {
        ...
        while (!conclusionDirection.spatial) {
            ...
            for (let i = 0; i < words.length - 1; i++) {
                ...
                if (this.settings.enableNegation && coinFlip()) {
                    question.premises.push(
                        `<span class="subject">${words[i + 1]}</span> ${timeName} <span class="is-negated">${(DIRECTION_NAMES_3D_INVERSE as any)[dirName]}</span> of <span class="subject">${words[i]}</span>`,
                    );
                } else {
                    question.premises.push(
            ...
            conclusionDirection = findDirection4D(
                wordCoordMap[words[0]],
                wordCoordMap[words[length - 1]],
            );
        }
        ...
        question.wordCoordMap = wordCoordMap;
        question.isValid = coinFlip();
        const oppositeDirection = findDirection4D(
            wordCoordMap[words[length - 1]],
            wordCoordMap[words[0]],
        );
        const direction = question.isValid
    ...
    createAnalogy(length: number) {
        ...
        if (analogyEnables.reduce((a, c) => a + +c, 0) < 1) {
            throw new Error(
                'Needs at least one of' + analogyEnables.join(', '),
            );
        }
        ...
        switch (choiceIndex) {
            ...
            case 3:
                while (flip !== isValidSame) {
                    ...
                    [a, b, c, d] = pickUniqueItems(
                        Object.keys(question.wordCoordMap),
                        4,
                    ).picked;
                    question.conclusion += `<span class="subject">${a}</span> to <span class="subject">${b}</span>`;

                    const dirA = findDirection(
                        question.wordCoordMap[a] as any,
                        question.wordCoordMap[b] as any,
                    );
                    const dirB = findDirection(
            ...
            case 5:
                while (flip !== isValidSame) {
                    ...
                    const { spatial, temporal } = findDirection4D(
                        question.wordCoordMap[a] as any,
                        question.wordCoordMap[b] as any,
                    );
                    const { spatial: spatial2, temporal: temporal2 } =
                        findDirection4D(
                            question.wordCoordMap[c] as any,
                            question.wordCoordMap[d] as any,
                        );
                    isValidSame =
                        spatial === spatial2 && temporal === temporal2;
                }
                break;
        }
    ...
    createBinary(length: number) {
        ...
        if (this.settings.syllogism[0]) {
            pool.push((length: number) => this.createSyllogism(length));
        }
        if (this.settings.distinction[0]) {
            pool.push((length: number) => this.createDistinction(length));
        }
        if (this.settings.comparisonNumerical[0]) {
        ...
        if (this.settings.direction[0]) {
            pool.push((length: number) => this.createDirection(length));
        }
        if (this.settings.direction3D[0]) {
            pool.push((length: number) => this.createDirection3D(length));
        }
        if (this.settings.direction4D[0]) {
            pool.push((length: number) => this.createDirection4D(length));
        }
